...
{
  struct list_elem *el;
  for (el = list_begin(&all_list); 
      el != list_tail(&all_list); 
      el = list_next(el))
  {
    struct thread *cur = list_entry(el, struct thread, allelem);
...
struct thread *get_child_thread( tid_t thread_id)
{
  struct thread* cur = thread_current();
  struct list_elem *el;
  for (el = list_begin(&cur->child_list); 
      el != list_end(&cur->child_list);
      el = list_next(el))
  {
    struct thread *temp = list_entry(el, struct thread, child_elem);
